...
| 이름 | 역할 | 담당 |
|---|---|---|
| **신예원** | 팀장 / Frontend | 홈·검색, 설명서 목록·상세·작성, 개인 저장소 |
| **김유겸** | Backend | 설명서·단계 이미지, 좋아요·저장, 마이페이지 API |
| **임현아** | Frontend | 로그인·회원가입, 회원 정보, 가족 연동 화면 |
| **김민서** | Backend | 회원·세션 인증, 가족 관계 API, 초기 구조 설계 |
| **이주헌** | Backend / Frontend | 질문·답변, 답변 선택·설명서 승격, 질문 화면, 이미지 주석 연동 |
