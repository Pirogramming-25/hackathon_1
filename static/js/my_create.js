...
async function loadMyGuides(page) {
  const guideList = document.querySelector("#guide-list");
  const pagination = document.querySelector("#pagination");

  try {
    // 🎯 백엔드의 '내가 쓴 글' API 호출
    const response = await fetch(`/api/users/me/guides/?page=${page}`, {
      method: "GET",
      credentials: "include",
    });
    ...
    if (!response.ok || !result?.success) {
      throw new Error(
        result?.message || "내가 쓴 글 목록을 불러오지 못했습니다."
      );
    }

    const guides = result.data.results || result.data;
    currentPage = page;

    if (guides.length === 0) {
      guideList.innerHTML = "<p>직접 작성한 설명서가 없습니다.</p>";
