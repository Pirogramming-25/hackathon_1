...
from questions.models import Answer # Answer 모델 추가 import

class GuideViewSet(viewsets.ModelViewSet):
    queryset = Guide.objects.all() # 기본 쿼리셋 설정
    serializer_class = GuideSerializer
    permission_classes = [IsAuthorOrReadOnly]
    parser_classes = [MultiPartParser, FormParser]

    # [신규 추가] 답변 승격 엔드포인트: /api/guides/answers/<answer_id>/promote/
    @action(detail=False, methods=['post'], url_path='answers/(?P<answer_id>\d+)/promote')
    def promote_answer(self, request, answer_id=None):
        try:
            answer = Answer.objects.get(id=answer_id)
        except Answer.DoesNotExist:
            return Response({"error": "존재하지 않는 답변입니다."}, status=404)

        if answer.author != request.user:
            return Response({"error": "본인이 작성한 답변만 등록할 수 있습니다."}, status=403)

        # 시리얼라이저 호출 (request.data + answer 객체를 context로 전달)
        ...
        if serializer.is_valid():
            ...
            return Response({"success": True, "message": "성공적으로 승격되었습니다."}, status=201)
    ...
    def get_queryset(self):
        ...
        if category: queryset = queryset.filter(category=category.upper())
        if search: queryset = queryset.filter(title__icontains=search)

        sort = self.request.query_params.get('sort', 'latest')
        if sort == 'most_scrapped': queryset = queryset.order_by('-scrap_count', '-created_at')
    ...
    def finalize_response(self, request, response, *args, **kwargs):
        ...
        if 200 <= response.status_code < 300:
            ...
            elif self.action == 'update' or self.action == 'partial_update': message = "설명서가 성공적으로 수정되었습니다."
